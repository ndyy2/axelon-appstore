           ...
           <div className="bg-surface-base border border-border-base rounded-2xl p-6 shadow-xl relative overflow-hidden group">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                <div className="flex items-center gap-5">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border shadow-inner ${appUpdate?.available ? 'bg-amber-500/10 border-amber-500/20 text-amber-500 animate-pulse' : 'bg-accent/5 border-accent/10 text-accent'}`}>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-7 h-7">
                      <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
                      {appUpdate?.available ? <circle cx="12" cy="12" r="3" fill="currentColor"/> : <path d="m9 12 2 2 4-4"/>}
                    </svg>
                  </div>
                  <div className="flex flex-col">
                    <h4 className="font-bold text-text-main text-lg mb-0.5">
                      {appLoading ? 'Checking system status...' : (appUpdate?.available ? 'Update Available' : 'System Up to Date')}
                    </h4>
                    <p className="text-sm text-text-dim">
                      {appLoading ? 'Connecting to GitHub...' : (appUpdate?.available ? `New version v${appUpdate.latest} is ready` : `You are running the latest version (v${appUpdate?.current || '1.0.0'})`)}
                    </p>
                  </div>
                </div>

                {!appLoading && appUpdate?.available && (
                  <button 
                    onClick={() => window.open(appUpdate.url, '_blank')}
                    className="px-6 py-2.5 rounded-xl bg-accent text-white font-bold text-sm shadow-lg shadow-accent/20 hover:scale-105 active:scale-95 transition-all"
                  >
                    View on GitHub
                  </button>
                )}
              ...
              {appUpdate?.available && appUpdate.notes && (
